...
# 检查Python包
echo "检查Python依赖包..."
python3 -c "import matplotlib" 2>/dev/null && echo "✓ matplotlib" || echo "✗ matplotlib - 运行: pip3 install matplotlib"
python3 -c "import pandas" 2>/dev/null && echo "✓ pandas" || echo "✗ pandas - 运行: pip3 install pandas"
python3 -c "import numpy" 2>/dev/null && echo "✓ numpy" || echo "✗ numpy - 运行: pip3 install numpy"
...
echo ""
echo "步骤1.5: 检查中文字体支持..."
cd scripts && python3 install_chinese_fonts.py && cd ..
...
echo ""
echo "🚀 下一步:"
echo "  查看 results/complete_analysis_report.txt 获取详细分析结论"
echo "  查看 results/ 目录中的图表文件可视化分析结果"
echo ""
echo "🔧 如果图表中文显示为方框:"
echo "  运行: cd scripts && python3 install_chinese_fonts.py"
echo "  查看建议的解决方案"
